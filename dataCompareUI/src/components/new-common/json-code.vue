  ...
  setup(props, { emit }) {
    ...
    const formatJSON = value => {
      try {
        ...
        // 使用 JSON.stringify 格式化并返回
        return JSON.stringify(parsed, null, 2); // 缩进 2 个空格
    ...
    onMounted(() => {
      ...
      const formattedState = formatJSON(initialState); // 格式化 JSON
    ...
    watch(
      ...
      newVal => {
        ...
        const formattedNewVal = formatJSON(normalizedNewVal); // 格式化新值

        if (editorView.value && currentValue !== formattedNewVal) {
          editorView.value.dispatch({
            changes: { from: 0, to: editorView.value.state.doc.length, insert: formattedNewVal }
          });
        }
      }
    );
