...
const model = defineModel<Api.VerifyConfigManage.VerifyConfigSearchParams>('model', { required: true });

async function reset() {
  model.value.configKey = null;
      ...
      <NCollapseItem :title="$t('common.search')" name="verifyData-search">
        <NForm ref="formRef" :model="model" label-placement="left" :label-width="100">
          <NGrid responsive="screen" item-responsive>
            <NFormItemGi
              span="24 s:12 m:6"
              :label="$t('page.contrast.verifyConfig.configKey')"
              path="taskName"
              class="pr-24px"
            >
