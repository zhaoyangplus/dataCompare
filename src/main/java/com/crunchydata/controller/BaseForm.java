...
import com.crunchydata.system.util.ServletUtils;
import com.crunchydata.utils.PageUtils;
...
public class BaseForm {
    ...
    protected QueryWrapper<?> pageQueryWrapperCustom(Map<String, Object> map, QueryWrapper<?> queryWrapper) {
        ...
        colQueryMap.forEach((k, v) -> {
            String column = StrUtil.toUnderlineCase(k);
            String value = StrUtil.toString(v).toUpperCase(); // 参数转大写
            ...
            switch (k) {
                ...
                case "datasourceName":
                    ...
                    queryWrapper.and(wrapper ->
                            wrapper.likeRight("UPPER(" + column + ")", value)
                                    .or()
                                    .likeLeft("UPPER(" + column + ")", value)
                                    .or()
                                    .like("UPPER(" + column + ")", value)
                    );
                    break;
                case "type":
                    //目标端过滤dm数据源
